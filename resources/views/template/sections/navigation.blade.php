<div class="text-white flex justify-between h-76px items-center">
        <div class="font-logo text-6xl">
            <a href="/">Car Blog</a>
        </div>
        <div class="text-2xl flex h-full items-center">
            <ul class="flex justify-between items-center h-full w-250px
            [&>li]:h-full [&>li]:flex [&>li]:items-center [&>li]:px-1.5 [&>li]:duration-300">
                <li class="hover:bg-background"><a href="/">Home</a></li>
                <li class="hover:bg-background"><a href="/about">About</a></li>
                <li class="hover:bg-background"><a href="/posts">Blog</a></li>
            </ul>
        </div>
</div>
